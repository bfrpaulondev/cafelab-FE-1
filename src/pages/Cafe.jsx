...
export default function Cafe({ bitems }) {
  ...
  useEffect(() => {
    const RetrieveBoutique = async () => {
      try {
        ...
        const boutique = data.data.filter((item) => item.section === "CAFE");
  ...
  return (
    ...
    >
      <Header />
      <HeroContactUs label={"Café"} link={"/cafe"} />
      <WebLocation name={"Café"} link={"/cafe"} />
      {loading && (
        <Center height="100vh">
          <Spinner size="xl" />
        </Center>
